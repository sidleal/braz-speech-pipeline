...
class AudioToTextSegmentsConverter():
    ...
    def __init__(self, output_path: Path, whisperx_model: FasterWhisperPipeline, batch_size: int = 8, compute_type: str = "float16"):
    ...
    def diarize_and_transcribe(self, audio_name: str, audio, corpus_id: int):
        data = {
            'audio_name': [],
            'start': [],
            'end': [],
        ...
        with Database() as db:
            with tempfile.NamedTemporaryFile(suffix=".wav") as temp_audio_file:
                ...
                self._save_segments(audio_id, audio_name, segments, data, db, temp_audio_file)
                
                df = pd.DataFrame(data)
                df.to_csv(self.OUTPUT_PATH / "summary.csv", index=False, encoding='utf-8', sep='|', mode='w')
    ...
    def _generate_text_segments(self, audio_name) -> List[SegmentWithSpeaker]:
        ...
        transcription_result: TranscriptionResult = self.whisperx_model.transcribe(audio, batch_size=self.batch_size)

        logger.debug("Aligning audio")
        model_a, metadata = whisperx.load_align_model(language_code=transcription_result["language"], device=self.device)
        ...
        diarize_model = whisperx.DiarizationPipeline(use_auth_token=CONFIG.pyannote.auth_token, device=self.device)
    ...
    def _save_segments(self, audio_id, audio_name, segments: List[SegmentWithSpeaker], data: "dict[str, list[Any]]", db: Database, temp_audio_file):
        ...
        for i, segment in enumerate(segments):
            try:
                ...
                segment_name = f'{i:04}_{audio_name}_{start_time}_{end_time}'

                transc_path = os.path.join(output_transcription_folder, f'{segment_name}.txt')
                transcription = segment['text']
                transcription = transcription.replace("'", "\'")
                with open(transc_path, "w", encoding="utf-8") as f:
                    f.write(transcription)

                segment_path_on_local = os.path.join(output_audio_folder, f'{segment_name}.wav')
                audio_segment = AudioSegment.from_wav(temp_audio_file)[int(start_time * 1000):int(end_time * 1000)]
                ...
                db.add_audio_segment(segment_path_on_local, transcription, audio_id, i, frames, duration, start_time, end_time, speaker_id)
