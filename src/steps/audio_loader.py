...
class AudioLoaderGoogleDrive:
    ...
    def load_and_downsample(
        ...
        _, non_silent_indexes = librosa.effects.trim(audio, top_db=20)
        peak = np.abs(audio).max()
        if peak > 1.0:
            audio = 0.98 * audio / peak

        return audio, non_silent_indexes

    def get_audio_from_mp4(
        self, file_id: str, format: Literal[".wav", ".mp4"]
        ...
        audio, sampling_rate = librosa.load(
            audio_filename, sr=self.sample_rate, mono=self.mono_channel
        )
